...
<cfcomponent name="MeldGalleryRemoteAOP" 
			displayname="MeldGalleryRemoteAOP:RemoteProxyBean" 
			hint="Abstract Base Class for Aop Based Remote Proxy Beans" 
			output="false">
			
	<cfset variables.proxyId = CreateUUId() />
	<cfset variables.beanFactoryName = "MeldGalleryBeanFactory" />
		...
		<cflock name="RemoteProxyBean.#variables.proxyId#.Setup" type="readonly" timeout="5">
			<cfif not StructKeyExists(variables, "constructed") or not variables.constructed>
				...
				<cftry>		
					...
					<cfset remoteFactory = bf.getBean("&MeldGalleryRemoteAOP") />
					<cfset variables.target = bf.getBean("MeldGalleryRemoteAOP") />
...
<cffunction name="getDisplayList" access="remote" returntype="any" output="false" > 
<cfset var rtn = callMethod('getDisplayList', arguments) />
<cfif isDefined('rtn')><cfreturn rtn /></cfif>
</cffunction>
